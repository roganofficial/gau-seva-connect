        ...
        <div className="flex items-start">
          <div className="flex-shrink-0">
            <div className="w-12 h-12 rounded-full bg-gaushala-green-100 flex items-center justify-center">
              {image ? <img src={image} alt={name} className="w-full h-full rounded-full object-cover" /> : <span className="text-gaushala-green-600 font-semibold text-lg">
                  {name.charAt(0)}
                </span>}
            </div>
          </div>
      ...
      <main className="flex-grow">
          ...
          <div className="container mx-auto px-4">
            <div className="flex justify-between items-center mb-8">
              ...
              <div className="flex items-center">
                ...
                <div onClick={handleToggleAnonymous} className="relative inline-block w-12 h-6 rounded-full cursor-pointer bg-lime-500">
                  <input type="checkbox" id="show-anonymous" className="sr-only" checked={showAnonymous} onChange={handleToggleAnonymous} />
                  <span className={`absolute left-1 top-1 w-4 h-4 rounded-full transition-transform duration-200 ease-in-out bg-white shadow-sm ${showAnonymous ? 'transform translate-x-6 bg-gaushala-green-500' : 'bg-gray-500'}`} />
                </div>
              </div>
            </div>
            ...
            <Tabs defaultValue="platinum">
              ...
              <TabsContent value="platinum">
                <div className="grid grid-cols-1 gap-6">
                  {platinumDonors.map((donor, index) => <DonorCard key={index} {...donor} />)}
                </div>
              </TabsContent>
              
              <TabsContent value="gold">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {goldDonors.map((donor, index) => <DonorCard key={index} {...donor} />)}
                </div>
              </TabsContent>
              
              <TabsContent value="silver">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {silverDonors.map((donor, index) => <DonorCard key={index} {...donor} />)}
                </div>
              </TabsContent>
            </Tabs>
          </div>
        ...
        <section className="py-16 bg-gray-50">
          <div className="container mx-auto px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {impactStories.map(story => <Card key={story.id} className="overflow-hidden">
        ...
        <section className="py-12 bg-gaushala-green-600 text-white">
          <div className="container mx-auto px-4 text-center">
            ...
            <a href="/donate" className="inline-block bg-white text-gaushala-green-700 hover:bg-gray-100 px-8 py-3 rounded-md font-semibold text-lg transition-colors">
              Become a Donor
            </a>
          </div>
        </section>
